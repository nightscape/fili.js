...
export default class IirFilter implements Filter {
    cf: any[] = [];
    cc: any[] = [];
    cone = {
      re: 1,
    ...
    biquadResponse(params: any, s: any) {
      ...
      const q = complex.add(this.cone, complex.mul(z, complex.add(s.a1, complex.mul(s.a2, z))));
    ...
    calcResponse(params: any) {
      ...
      for (let cnt = 0; cnt < this.cf.length; cnt++) {
        const r = this.biquadResponse(params, this.cf[cnt]);
