...
export default class IirCoeffs {
    ...
    fromPZ(params: any) {
      const coeffs = IirCoeffs.initCoeffs();
      coeffs.a0 = 1;
      coeffs.b.push(1);
    ...
    lowpass(params: any) {
      ...
      const p = IirCoeffs.preCalc(params, coeffs);
      if (params.preGain) {
    ...
    peak(params: any) {
      const coeffs = IirCoeffs.initCoeffs();
      const p = IirCoeffs.preCalcGain(params);
      coeffs.k = 1;
    ...
    lowshelf(params: any) {
      ...
      const p = IirCoeffs.preCalcGain(params);
      coeffs.k = 1;
      const sa = 2 * Math.sqrt((p as any).A) * (p as any).alpha;
