'use strict';
import IirCoeffs, {Coeffs, IirParams} from './iirCoeffs';

const getCoeffs = new IirCoeffs();
const table: { bessel: { [key: string] : number[][]}} = {
...
const tiTable: {[key: string]: ValTable} = {
  bessel: {
    as: [
      [1.3617],
      [1.3397, 0.7743],
...
export function calcCoeffs(params: IirParams, behavior: string): Coeffs[] {
  const filter = [];
  const cnt = 0;
  if (behavior !== 'fromPZ') {
    if (params.order > 12) {
...
function initCalcCoeffs(behavior: string) {
